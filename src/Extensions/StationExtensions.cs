...
{
	private static Dictionary<int, string> CountryCodes = new Dictionary<int, string>()
	{
		{1, "ru"},
		{2, "ua"},
		{3, "us"},
	...
	public static MarkupString GetCountryIcon(this Station station)
	{
		...
		if (station.CountryId.HasValue && CountryCodes.ContainsKey(station.CountryId.Value))
		{
			var countryCode = CountryCodes[station.CountryId.Value];
			sb.AppendLine($"<title>{countryHelper.GetNameByCountryCode(countryCode)}</title>");
			var data = countryHelper.GetNameByCountryCode(countryCode);
